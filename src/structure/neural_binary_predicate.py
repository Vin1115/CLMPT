...
class NeuralBinaryPredicate:
    ...
    def get_all_entity_rankings(self, batch_embedding_input, eval_batch_size=16, score='cos'):
        ...
        for begin in range(0, batch_size, eval_batch_size):
            ...
            if score == 'cos':  # TODO [16, 14505] negative_union_logit = torch.max(negative_union_logit, dim=1)[0]  # [1,2,14505] -> [1,14505]
    ...
    def get_all_entity_rankings_dnf(self, batch_embedding_input1, batch_embedding_input2, eval_batch_size=16, score='cos'):
        ...
        for begin in range(0, batch_size, eval_batch_size):
            ...
            if score == 'cos':
                ...
                ranking_score1 = - torch.norm(eval_batch_embedding_input1 - self.entity_embedding, dim=-1)
                ranking_score2 = - torch.norm(eval_batch_embedding_input2 - self.entity_embedding, dim=-1)
            ranking_score = torch.stack((ranking_score1, ranking_score2), dim=1)  # [16,2,14505]
            ranking_score = torch.max(ranking_score, dim=1)[0]  # [16,2,14505] -> [16,14505]
            ranked_entity_ids = torch.argsort(ranking_score, dim=-1, descending=True)
            entity_rankings = torch.argsort(ranked_entity_ids, dim=-1, descending=False)
            entity_ranking_list.append(entity_rankings)
