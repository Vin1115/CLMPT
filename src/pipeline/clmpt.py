...
class CLMPTLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, nbp: NeuralBinaryPredicate, layers=2, agg_func='mean', pre_norm=True):
        ...
        if pre_norm:
            ...
            self.transformer = nn.TransformerEncoder(encoder_layer, self.num_encoder_layers, encoder_norm)  # post_norm
...
class CLMPTReasoner(Reasoner):
    def __init__(self,
                 nbp: NeuralBinaryPredicate,
                 lgnn_layer: CLMPTLayer,
                 depth_shift=0):
        self.nbp = nbp
