...
q2b_lstr2name = {}
for s, n in zip(q2b_lstr_list, q2b_names):
    print(s, n)
    q2b_lstr2name[
...
def align_entities_relations(labeled_q2b_type, q2b_sample) -> Dict:
    d = {}
    def _align(labeled_q2b_type, q2b_sample):
        for sub_type, sub_sample in zip(labeled_q2b_type, q2b_sample):
            if not isinstance(sub_type, str):
                _align(sub_type, sub_sample)
            else:
                if sub_type[0] in 'sr':
                    d[sub_type] = sub_sample
    _align(labeled_q2b_type, q2b_sample)
    return d

def convert_q2b_folder(q2b_folder, output_folder):
    """
    ...
    kgidx = KGIndex()
    with open(osp.join(q2b_folder, 'ent2id.pkl'), 'rb') as f:
    ...
    with open(osp.join(q2b_folder, 'rel2id.pkl'), 'rb') as f:
    ...
    train_kg = KnowledgeGraph.create(
        triple_files=osp.join(q2b_folder, 'train.txt'),
        kgindex=kgidx)
    ...
    # train queries
    with open(osp.join(q2b_folder, "train-queries.pkl"), 'rb') as f:
        train_queries = pickle.load(f)

    with open(osp.join(q2b_folder, "train-answers.pkl"), 'rb') as f:
        train_answers = pickle.load(f)

    lstr_xy_dict = {}
    for key, labeled_type, lstr in zip(
        q2b_types_key_list, labeled_q2b_types_list, q2b_lstr_list):
    ...
    # valid queries
    with open(osp.join(q2b_folder, "valid-queries.pkl"), 'rb') as f:
        valid_queries = pickle.load(f)

    with open(osp.join(q2b_folder, "valid-easy-answers.pkl"), 'rb') as f:
        valid_easy_answers = pickle.load(f)

    with open(osp.join(q2b_folder, "valid-hard-answers.pkl"), 'rb') as f:
    ...
    # test queries
    with open(osp.join(q2b_folder, "test-queries.pkl"), 'rb') as f:
        test_queries = pickle.load(f)

    with open(osp.join(q2b_folder, "test-easy-answers.pkl"), 'rb') as f:
        test_easy_answers = pickle.load(f)

    with open(osp.join(q2b_folder, "test-hard-answers.pkl"), 'rb') as f:
...
if __name__ == "__main__":
    q2b_folder = "./q2b_data/q2b-dataset/{}"
    output_folder = "./q2b_data/{}"

    for dataset in [
        "FB15k-237-q2b",
        "NELL-q2b"]:
        print(dataset)
        convert_q2b_folder(q2b_folder.format(dataset),
